...
import { getLocalMemory, deleteLocalMemory, type LocalMemory } from "@/src/utils/localMemories";
import { loadLocalPhotos, photoToBase64 } from "@/src/utils/localPhotos";
...
export default function MemoryDetailScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const m = await getLocalMemory(id as string);
      if (!m) { show("Memory not found", "error"); router.back(); return; }
      setMemory(m);
    } catch {
  ...
  const onSave = async () => {
    ...
    setBusy(true);
    try {
      // Read local file as base64 for saving to device gallery
      const { FileSystem } = await import("expo-file-system");
      const b64 = await FileSystem.readAsStringAsync(memory.uri, { encoding: FileSystem.EncodingType.Base64 });
      const res = await saveToDevice(b64);
      if (res === "ok") show("Saved to your device", "success");
      else if (res === "unsupported") show("Saving isn't available here", "info");
      else if (res === "blocked") { show("Enable photo access in Settings", "error"); Linking.openSettings(); }
  ...
  const onRegenerate = async () => {
    ...
    try {
      // Load source photos as base64
      const allPhotos = await loadLocalPhotos();
      const sourcePhotos = allPhotos.filter((p) => memory.source_photo_ids.includes(p.id));
      if (sourcePhotos.length === 0) { show("Source photos no longer available", "error"); setRegenerating(false); return; }

      const images_b64 = await Promise.all(sourcePhotos.map((p) => photoToBase64(p.uri)));
      const result = await api<{ id: string; title: string; image_b64: string }>(
        "/memories/generate",
        { method: "POST", body: { prompt: memory.prompt, images_b64 } }
      );

      await saveMemoryLocally(result.id, memory.prompt, result.title, result.image_b64, memory.source_photo_ids);
  ...
  return (
    <View style={styles.container} testID="memory-detail-screen">
      <StatusBar style="light" />
      <Image source={{ uri: memory.uri }} style={StyleSheet.absoluteFill} contentFit="cover" transition={250} />
      ...
      <BlurView intensity={50} tint="dark" style={[styles.sheet, { paddingBottom: insets.bottom + spacing.lg }]}>
        ...
        <Text style={styles.prompt} numberOfLines={3}>{memory.prompt}</Text>
...
function Action({ icon, label, onPress, disabled, testID }: {
  icon: keyof typeof Ionicons.glyphMap; label: string; onPress: () => void; disabled?: boolean; testID: string;
}) {
  return (
    <Pressable testID={testID} onPress={onPress} disabled={disabled} style={({ pressed }) => [styles.action, pressed && { opacity: 0.7 }, disabled && { opacity: 0.5 }]}>
      <View style={styles.actionIcon}><Ionicons name={icon} size={22} color="#FFFFFF" /></View>
...
const styles = StyleSheet.create({
  ...
  roundBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: "rgba(25,24,24,0.4)", alignItems: "center", justifyContent: "center" },
  sheet: { position: "absolute", left: 0, right: 0, bottom: 0, borderTopLeftRadius: radius.lg, borderTopRightRadius: radius.lg, paddingHorizontal: spacing.xl, paddingTop: spacing.xl, overflow: "hidden" },
