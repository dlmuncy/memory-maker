...
import { useCreate, Photo } from "@/src/context/CreateContext";
...
export default function CreatePhotosScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const data = await api<Photo[]>("/photos");
      setPhotos(data);
    } catch {
  ...
  const handlePicked = async (images: string[]) => {
    ...
    try {
      ...
      for (const b64 of images) {
        const p = await api<Photo>("/photos", { method: "POST", body: { image_base64: b64 } });
        added.push(p);
      }
  ...
  const renderItem = ({ item, index }: { item: Photo; index: number }) => {
    ...
    return (
      ...
      >
        <Image source={{ uri: toDataUri(item.image_base64) }} style={styles.tileImage} contentFit="cover" />
        {isSel ? (
    ...
    <View style={styles.container} testID="create-photos-screen">
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>No photos yet — tap “Add new” to upload some.</Text>
          }
...
const styles = StyleSheet.create({
  ...
  uploadingText: { color: "#FFFFFF", fontFamily: font.medium, fontSize: type.lg },
});
