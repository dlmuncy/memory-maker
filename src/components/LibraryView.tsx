...
export default function LibraryView({ subjects, onAddSubjectClick, onSubjectClick, vaultUnlocked }: LibraryViewProps) {
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      <header className="text-center md:text-left max-w-2xl space-y-4">
        ...
        <p className="text-body-lg text-on-surface-variant leading-relaxed">A curated collection of uploaded subjects. Each portrait acts as a reusable anchor when you compose a new memory.</p>
      </header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {sortedSubjects.map((subject) => (
          <button type="button" key={subject.id} onClick={() => onSubjectClick(subject)} className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-6 flex flex-col justify-between group hover:bg-surface-container-low hover:shadow-md transition-all h-[420px] text-left">
            <span className="space-y-4 block">
              ...
              <span className="block"><strong className="text-headline-md text-primary group-hover:text-secondary transition-colors block">{subject.name}</strong><span className="text-body-md text-on-surface-variant mt-1 block">Relationship: {subject.relationship}</span></span>
            </span>
            <span className="mt-6 flex justify-between items-center border-t border-outline-variant pt-4"><span className="text-label-sm text-outline uppercase tracking-widest">Memory Recalls</span><strong className="text-label-md text-secondary text-lg bg-secondary-container/30 px-3 py-0.5 rounded-full">{subject.recalls}</strong></span>
          </button>
        ))}
        ...
        <button type="button" onClick={onAddSubjectClick} className="border-2 border-dashed border-outline-variant bg-surface-container-low rounded-2xl p-6 flex flex-col items-center justify-center group hover:border-primary hover:bg-surface-container-high h-[420px]">
          ...
          <span className="text-label-sm text-on-surface-variant mt-4 text-center max-w-[200px] leading-relaxed uppercase tracking-widest">Upload a new portrait anchor</span>
        </button>
      </div>
    </div>
  );
